...
os.makedirs("shap_summary_plots", exist_ok=True)
...
for i in range(num_outputs):
    ...
    save_path = f"./shap_summary_plots/shap_summary_{output_names[i]}.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()

for i in range(num_outputs):
    print(f"绘制 SHAP Summary 重要性图：{output_names[i]}")
    plt.figure(figsize=(12, 4))  # 显式创建新图形
    fig = plt.gcf()
    shap.summary_plot(shap_vals_for_output_i, test_samples.cpu().numpy(), feature_names=feature_columns,
                      plot_type="bar", color=colors)
    plt.xticks(fontname='Times New Roman', fontweight='bold', fontsize=22)
    plt.yticks(fontname='Times New Roman', fontweight='bold', fontsize=22)

    bar_save_path = f"./shap_summary_plots/shap_summary_importance_{output_names[i]}.png"
    fig.savefig(bar_save_path, dpi=300, bbox_inches='tight')
